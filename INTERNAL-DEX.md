...
**Docs snapshot:** **0.11.698** (stamp **w289**)
...
|---------|-------------------|---------------------|--------------|
| **Internal DEX** | Smart Wallet → **Swap** | Jupiter (Solana); EVM: LiFi → 0x → Uni V2 → V3 | Smart Wallet **0.45%** atomic on fee-bearing path; LiFi EVM currently **+0.25%** LI.FI = **0.70%** combined |
...
Swap panel (app.js + swap-manager façade)
  ...
  Quote coordinator  ── sequential, never parallel ──
        ...
        │
        └─ EVM     → sequential providers (live MODE Worker for LiFi)
                       │
                       └─ 1) LiFi Worker /v1/lifi/quote (45 bps) → 2) 0x → 3) Uni V2 → 4) Uni V3
                          (best-effort fees — see §4)
        │
        ▼
...
| **Official routers + 0x** | Live order: LiFi (45 bps) → 0x → Uniswap V2 → Uniswap V3. Sequential; no parallel fan-out. |
...
```text
1. LiFi          Production Worker /v1/lifi/quote (+ routes only if needed) — 45 bps
                 ← ETH / Polygon / Base / BSC / RH / Arb / OP / Avalanche
2. 0x            next sequential provider when LiFi has no usable quote
3. Official V2   Uniswap V2 (official router)
4. Official V3   Uniswap V3 (official router)
```

**Arbitrum / Optimism / Avalanche Internal DEX (0.11.156–159 history; live w289):** EVM quote order is LiFi → 0x → Uni V2 → V3. Early probes used LiFi as the primary coordinator (ETH→USDC on 42161 and 10; AVAX→USDC on 43114). Do **not** treat any single provider’s chain allowlist as complete support. Token addresses are per-chain (see [CHAINS.md](./CHAINS.md)).
