...
**Live product / architecture snapshot:** **0.11.698** (stamp **w289**)
...
**Last architecture update:** 2026-09-21 (0.11.698 / w289; production LiFi MODE; public RPC default; 0.11.159 body kept as the shipped-module snapshot)  
...
| Platform fees | **0.45%** SW swap · **0.85%** SW bridge · LiFi **0.25%** display-only (not SW treasury; combined **0.70%** / **1.10%**) · `allowsSignAfterFeeFailure` · Jupiter fee-free rebuild OK · LiFi carve-out only · EVM quotes LiFi→0x→V2→V3 · none on Send / external DEX |

**No Smart Wallet cloud custody.** Network calls go from the user’s browser to public RPCs / price APIs / optional user Helius / WalletConnect / Onramper. Live LiFi MODE is the **production** Cloudflare Worker `https://smart-wallet-lifi-proxy.smart-wallet.workers.dev` (service **1.7.0**; staging same version). LiFi uses verify-host / proxy-config / transport — **not** main-thread-only. Also in production: market-data Worker and Solana ALT verifier. Mail uses `https://smart-wallet-rpc-gateway.smart-wallet.workers.dev` (service **0.2.31**). Public-data paths **refuse secrets**. Staging remains an explicit developer option. Normal use is **public RPC** (production-managed RPC is blocked / not the live default). The extension never talks to `li.quest` with a key.


### UI load (w289)

- **Logos Host** ships on **CORE**. Holdings soft **no-op** if Logos Host is missing (no hard crash).
- Holdings list virtualization: row height **54px**, overscan **8**.
- Messaging and swap **heavy panels DEFER** — they are not pulled on every unlock.
- **Store vs unpacked:** Store package omits owner/dev tools; **ui-revamp** is not shipped.
