...
**Wallet version:** **0.11.698** (stamp **w289**)
**Production mail Worker:** **0.2.31** via `https://smart-wallet-rpc-gateway.smart-wallet.workers.dev` (`PUBLIC_FULL_SERVICE`; Managed RPC remains separate, **blocked**, and **not** the live default)
...
Related: [PRIVACY-POLICY.md](./Chrome-extension-store-for-reviewers/PRIVACY-POLICY.md) · [DOCUMENTATION.txt](./DOCUMENTATION.txt)
...
- Production mail is live on Worker **0.2.31** (health-verified on `smart-wallet-rpc-gateway`). Software-wallet send, receive, pull and privacy actions use production. Ledger send, receive, pull, block, enable and revoke are supported in production.
- Ledger **Delete for me**, **Report**, and **Request deletion of my server messages** use the production **0.2.31** authorization path (supersedes the earlier 0.2.29 `401` limitation and the reviewed-but-undeployed 0.2.30 note).
