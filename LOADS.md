...
**Last updated:** 2026-09-21 (live-feeds / budget / gateway / WS-health; Code Review max-use pass verified live `sw-request-budget.js` ceilings)  

**Live product stamp:** **0.11.698 / w289**. Normal use is **public RPC** (production-managed RPC is **blocked** / not the live default). LiFi goes through the **production Worker only** — the extension does **not** call `li.quest` with a key. **No continuous Helius** — History does not poll Helius in the background; optional user **custom Solana RPC** is the supported custom endpoint path.

**UI load notes (w289):** Logos Host is on **CORE**; holdings soft no-op if Logos Host is missing; holdings list virtualization **54px** row / overscan **8**. Messaging and swap **heavy panels DEFER** — not loaded on every unlock.
...
| **Workers** | Production: LiFi proxy **1.7.0**, mail via RPC-gateway Worker **0.2.31**, Solana ALT verifier, market-data. Staging is an explicit developer option. |
...
**Code Review 2026-09-21 max-use pass** verified **live code ceilings** from Lazarus `sw-request-budget.js` @ `9c3734e` (header / §0.1 / §12.3 above). **No new HAR** was captured; historical RPM / scale tables stay **historical**.
...
| **Send (preflight, no broadcast)** | **3–8** | **~12** | Balance, nonce/`getLatestBlockhash`, fee/gas, optional simulate. Screen budget **24/15 s** (live) but send is **critical** (bypass) |
...
| **Bridge execute** | quote + send + fee + status | status **≤50–90** | 10–12 s interval. 85 bps best-effort (never sign tainted fee tx) |
...
| **Mail / RPC-gateway** | `https://smart-wallet-rpc-gateway.smart-wallet.workers.dev` · **0.2.31** (production mail; Managed RPC blocked) |
...
### 12.3 Per-screen request budget (0.11.698 / w289 live) — non-critical only
...
| send | 24 | fee estimate (non-critical extras) |
...
| wallet | 32 | catch-all / default wallet bucket |
| solana | 32 | Solana-specific soft work (keeps EVM holdings from starving SPL discovery) |

**Gating (not Home-only):** soft budget checks run through `rpc-gateway.js` `applyRequestBudget`, `rpc-network-host.js` `B.allow`, and the Home `live-balance` tick. When the cap is hit, the extra **soft** ping is skipped (last cache stays). This is the main reason idle Home stays in the 45–70 band even if several timers wake together.
