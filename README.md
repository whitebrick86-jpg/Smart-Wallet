...
**Docs / privacy:** this repository · **Live product:** **0.11.698** (stamp **w289**) · **Product map:** [PRODUCT.md](./PRODUCT.md)
...
|--------------|----------|
| **All software** (seed / imported) | Rust/WASM always-on custody (`rustJsVaultForbidden`). Seeds remain encrypted at rest. Password ON/OFF as before. **No** plaintext popup `SESSION_VAULT_PASSWORD`. Resume uses AES-GCM envelope + offscreen CryptoKey; `passwordBytes` are transient only. Routine signing stays inside WASM. Seed removal remains an explicit **✕** action. |
| **Ledger** | Optional global Ledger password; keys stay on device |

Full detail: **DOCUMENTATION §15**, **§17.2**.
