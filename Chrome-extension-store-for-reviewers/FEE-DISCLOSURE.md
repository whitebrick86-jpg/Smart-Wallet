...
**Best-effort platform fees** (`allowsSignAfterFeeFailure` = **true**). Never block solely because fee insert failed if the route is otherwise safe. Never sign malformed / misdirected / unverifiable (tainted) fee payloads. Where the fee-bearing path applies, **45 bps** remains the Smart Wallet treasury rate. **Jupiter:** may rebuild a fresh fee-free quote and sign only that rebuild. **LiFi:** narrow fee-unavailable carve-out only — **no** Jupiter-style client fee-free rebuild.

**EVM internal swap quote order:** LiFi (45 bps) → 0x → Uniswap V2 → Uniswap V3 (sequential).
